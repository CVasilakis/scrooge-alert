...
- **Automated Execution:** Handled automatically by the systemd service (`skroutz-price-alert.timer`). It executes hourly, incorporating a randomized startup delay to obscure scheduling footprints.
- **Manual Execution:** Execute `./scripts/run_scraper.sh` to run the scraper interactively and view output logs in the terminal.
- **Testing Configurations:** Run `./scripts/run_scraper.sh --test-notification` to send a test payload and verify that the Apprise URLs in your `.env` file are configured correctly.
