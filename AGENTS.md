...
- **Installation:** Execute `./install.sh`. This script creates the Python virtual environment, installs dependencies from `requirements.txt`, and generates one hourly systemd user timer+service pair **per registered plugin** (`<plugin>-scraper.{service,timer}`), each invoking `run.sh --quiet --<plugin>`. Pass `--<plugin>` (e.g. `./install.sh --skroutz`) to (re)provision just one plugin's unit.
