...
- **Architecture:** The core application logic resides in `src/scraper/skroutz_price_alert.py`. The code is structured using a decoupled, layered architecture employing Dependency Injection. It is separated into distinct components: a Network Layer (`SkroutzClient`), Data Access Layer (`ProductsManager`), Application Orchestration Layer (`ScrapingOrchestrator`), Configuration & Health Layer (`ConfigValidator`), and CLI Layer (`CLIHandler`). User configuration is completely externalized to `.env` (notification endpoints) and `data/products.json` (the list of tracked products). It is designed to run silently as a background task, leveraging a local Python virtual environment (`venv`) to isolate dependencies.
...
- **Code Style:** Standard Python object-oriented practices are used with a focus on the Single Responsibility Principle and Dependency Injection. Logic is strictly separated into focused classes (e.g., `ProductsManager` for pure data I/O, `SkroutzClient` for network requests, and `ScrapingOrchestrator` for the main loop execution). Configuration and state are decoupled from the main execution logic.
